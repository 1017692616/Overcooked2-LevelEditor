...
`Tools > Build AssetBundles` 会构建项目里所有带 AssetBundle 标记的包，通常只有维护者整理全项目资源时才需要。日常导出单个地图请用 `Tools > Build Current Level AssetBundles`，它会在构建后自动安装到游戏目录，并且只复制真正的 bundle 文件，不会把 `.manifest` 或 `.meta` 一起装进去。
